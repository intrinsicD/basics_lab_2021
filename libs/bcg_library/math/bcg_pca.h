...
inline void least_squares_fit_svd(Pca<D> &pca, const MatrixS<N, D> &P, const VectorS<D> &mean) {
    Eigen::JacobiSVD<MatrixS<N, D>> svd((P.rowwise() - mean.transpose()) / std::sqrt(P.rows() - 1),
...
template<int N, int D>
inline void
weighted_least_squares_fit_svd(Pca<D> &pca, const MatrixS<N, D> &P, const VectorS<D> &mean, const VectorS<N> &weights) {
    Eigen::JacobiSVD<MatrixS<N, D>> svd(
            weights.asDiagonal() * (P.rowwise() - mean.transpose()) / std::sqrt(P.rows() - 1),
            Eigen::ComputeFullV | Eigen::ComputeFullU);
    pca.directions = svd.matrixV();
...
template<int N, int D>
inline void
weighted_least_squares_fit_eig(Pca<D> &pca, const MatrixS<N, D> &P, const VectorS<D> &mean, const VectorS<N> &weights) {
    MatrixS<N, D> P_hat = P.rowwise() - mean.transpose();
