...
#include "bcg_rotation_geodesic_median.h"
#include "bcg_rotation_geodesic_mean.h"

namespace bcg {

geodesic_median_so3::geodesic_median_so3(bool robust, bool outlier_reject)
        : m_robust(robust),
          m_outlier_reject(outlier_reject),
          m_R(MatrixS<3, 3>::Identity()) {}

geodesic_median_so3::geodesic_median_so3(std::vector<bcg_scalar_t> weights, bool robust, bool outlier_reject)
        : m_robust(robust),
          m_outlier_reject(outlier_reject),
          m_R(MatrixS<3, 3>::Identity()),
          m_weights(std::move(weights)) {}

geodesic_median_so3::geodesic_median_so3(const MatrixS<3, 3> &initial_estimate, bool outlier_reject)
        : m_robust(false),
          m_outlier_reject(outlier_reject),
          m_R(initial_estimate) {}

geodesic_median_so3::geodesic_median_so3(const MatrixS<3, 3> &initial_estimate, std::vector<bcg_scalar_t> weights,
                                         bool outlier_reject)
        : m_robust(false),
          m_outlier_reject(outlier_reject),
          m_R(initial_estimate),
...
MatrixS<3, 3>
geodesic_median_so3::operator()(const std::vector<MatrixS<3, 3>> &rotations, bcg_scalar_t eps,
                                int max_iterations) {
    if (m_robust) {
    ...
    while (true) {
        ...
        if (m_outlier_reject) {
            ...
            for (size_t i = 0; i < rotations.size(); ++i) {
                ...
                delta += item.second * m_weights[i] / item.first;
            ...
            for (size_t i = 0; i < rotations.size(); ++i) {
                VectorS<3> v = matrix_logarithm(rotations[i] * m_R.transpose());
                bcg_scalar_t length = v.norm();
                if (length > 0) {
                    delta += v * m_weights[i] / length;
