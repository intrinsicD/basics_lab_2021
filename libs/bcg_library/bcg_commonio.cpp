...
void print_info(const std::string &msg) { printf("%s\n", msg.c_str()); }
...
#ifdef _WIN32
    auto path8 = std::filesystem::u8path(filename);
    auto wmode = std::wstring(string{mode}.begin(), std::string{mode}.end());
...
static std::vector<std::string> split_cli_names(const std::string &name_) {
    auto name = name_;
...
std::string get_usage(const cli_state &cli) {
    auto type_name = [](const cli_option &option) -> std::string {
        auto str = std::string{};
        str += "<";
    ...
    auto def_string = [](const cli_option &option) -> std::string {
        if (option.req) return std::string{"[required]"};
        auto str = std::string{};
        str += "[";
        for (auto &value : option.def) {
            switch (value.type) {
                ...
                case cli_type::uinteger:
                    ...
                    break;
                case cli_type::number:
                    str += std::to_string(value.number);
                    break;
                case cli_type::string:
                    str += value.text;
                    break;
                case cli_type::boolean:
                    str += value.integer ? "true" : "false";
                    break;
            }
        }
        str += "]";
    ...
    auto usage_optional = std::string{}, usage_positional = std::string{};
    for (auto &option : cli.options) {
...
static bool parse_value(
        cli_value &value, const std::string &arg, const std::vector<std::string> &choices) {
    if (!choices.empty()) {
...
bool parse_cli(cli_state &cli, int argc, const char **argv, std::string &error) {
    ...
    for (auto &option : cli.options) {
        ...
        for (auto &name : split_cli_names(option.name)) {
            ...
            if (option.nargs == 0) {
                values = {name.find("--no-") == std::string::npos ? "true" : "false"};
                option.set = true;
