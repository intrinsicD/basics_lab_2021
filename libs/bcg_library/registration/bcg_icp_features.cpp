...
FeatureBatch::FeatureBatch(std::string batch_name, std::unordered_map<std::string, Feature> batch) : batch_name(std::move(batch_name)),
...
void FeatureBatch::build(const vertex_container &container, bool with_index){
    ...
    if(with_index){
        index = std::make_unique<kdtree_matrix<bcg_scalar_t>>(batch_embedding, 10);
    }
}
...
void Features::build(const vertex_container &container, bool with_indices){
    std::vector<MatrixS<-1, -1>> embeddings;
    for(auto &item : batches){
        item.second.build(container, with_indices);
    }
}
