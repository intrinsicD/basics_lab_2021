...
struct coherent_point_drift_base2 {
    ...
    bcg_scalar_t N_P, sigma_squared, kdtree_sigma_threshold, omega;
...
protected:
    MatrixS<-1, -1> X, Y;
    Vector<kernel_precision, -1> OnesN, OnesM;
    ...
    virtual void update_P_parallel(size_t parallel_grain_size = 1024);

    virtual void update_P_nystroem();

    virtual void update_P_nystroem_FGT();

    virtual void update_P_kdtree(size_t parallel_grain_size = 1024);
...
struct coherent_point_drift_rigid : public coherent_point_drift_base2 {
    bcg_scalar_t s;
    VectorS<3> t;
    MatrixS<3, 3> R;
...
struct coherent_point_drift_affine : public coherent_point_drift_base2 {
    VectorS<3> t;
    MatrixS<3, 3> B;
...
struct coherent_point_drift_nonrigid : public coherent_point_drift_base2 {
...
struct coherent_point_drift_bayes : public coherent_point_drift_base2 {
    CoherenceType coherence_type;
    bcg_scalar_t s;
    VectorS<3> t;
    MatrixS<3, 3> R;
...
private:
    ...
    void update_P_parallel(size_t parallel_grain_size = 1024) override;

    void update_P_nystroem() override;

    void update_P_nystroem_FGT() override;

    void update_P_kdtree(size_t parallel_grain_size = 1024) override;
