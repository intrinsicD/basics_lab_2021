...
void explicit_smoothing(halfedge_mesh &mesh, const mesh_laplacian &laplacian, int smoothing_steps,
    ...
    explicit_smoothing(mesh, laplacian, mesh.positions, smoothing_steps, parallel_grain_size);
...
void explicit_smoothing(halfedge_mesh &mesh, const mesh_laplacian &laplacian, property<bcg_scalar_t, 1> &property,
                              int smoothing_steps, size_t parallel_grain_size) {
                    ...
                    for (uint32_t i = range.begin(); i != range.end(); ++i) {
                        ...
                        if ((v_feature && v_feature[v]) || mesh.is_boundary(v)) {
                            ...
                            for (const auto vh : mesh.halfedge_graph::get_halfedges(v)) {
                                ...
                                smooth_value += e_weight[mesh.get_edge(vh)] * (property[tv] - property[v]);
                            }

                            if (v_weight[v] != 0) {
                                smooth_value = smooth_value / v_weight[v];
                            }
                            property[v] += 0.5 * smooth_value;
                        }
                    }
                }
        );
