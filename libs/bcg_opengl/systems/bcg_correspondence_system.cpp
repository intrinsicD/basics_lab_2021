...
void correspondence_system::on_correspondences_estiamte(const event::correspondences::estimate &event) {
    ...
    if (!state->scene.has<component_transform>(event.source_id)) return;
    if (!state->scene.has<component_transform>(event.target_id)) return;
    ...
    auto &src_model = state->scene.get<component_transform>(event.source_id);
    auto &target_model = state->scene.get<component_transform>(event.target_id);
