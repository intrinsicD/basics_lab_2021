...
void gui_segmented_jaw_alignment(viewer_state *state) {
    if (state->scene.valid(state->picker.entity_id)) {
        if (ImGui::CollapsingHeader("Compute Segmentation")) {
            ...
            if (compute_edge_scaling) {
                ...
                if (ImGui::Button("Compute Vertex Curvatures")) {
                    triggered_compute = true;
                    state->dispatcher.trigger<event::mesh::curvature::taubin>(state->picker.entity_id,
                                                                              post_smoothing_steps,
                                                                              two_ring_neighborhood);
                    ...
                    if (!result_property_name.empty()) {
                        ...
                        if (ImGui::Button("Compute Edge Scaling Function")) {
                            ...
                            if (vertex_property) {
                                ...
                                edge_from_vertex_function_min(mesh, "e_dijkstra_scaling", vertex_property_original,
                                ...
                                if (e_dijkstra_scaling) {
                                    auto result_shift = shift(&mesh.edges, e_dijkstra_scaling,
                                                              -MapConst(e_dijkstra_scaling).minCoeff());
                            ...
                            if (mesh.edges.has("e_laplacian_scaling")) {
                                ...
                                if (state->picker.mode == viewer_picker::Mode::vertices &&
                                    ...
                                    if (material.attributes[2].buffer_name.empty() && mesh.vertices.has("v_selected")) {
                                        ...
                                        state->dispatcher.trigger<event::points_renderer::set_point_size_attribute>(
                                                state->picker.entity_id, material.attributes[2]);
                                    }
                                }
                                if (mesh.vertices.has("v_harmonic_field")) {
                                    ...
                                    gui_property_clamp(state, &mesh.vertices, v_harmon_field, hf_clamp_min,
                                                       hf_clamp_max);
                                }
                            }
                        }
        ...
        if (ImGui::CollapsingHeader("Compute Alignment")) {
            ...
            ImGui::SameLine();
            if (ImGui::Button("Set as Source Jaw")) {
                if (state->scene.valid(state->picker.entity_id)) {
            ...
            ImGui::Separator();
            if (source != entt::null && state->scene.valid(source)) {
                ImGui::LabelText("source ", "%d", int(source));
            }
            if (source_jaw != entt::null && state->scene.valid(source_jaw)) {
            ...
            if (target != entt::null && state->scene.valid(target)) {
                ImGui::LabelText("Target ", "%d", int(target));
            }
            if (ImGui::CollapsingHeader(("Teeth " + std::to_string(source_teeth.size())).c_str())) {
