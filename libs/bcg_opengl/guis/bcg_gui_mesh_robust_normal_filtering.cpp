        ...
        if (state->scene.valid(id) && state->scene.has<halfedge_mesh>(id)) {
            ...
            switch (static_cast<NormalFilteringType>(e)) {
                case NormalFilteringType::unilateral_belyaev_ohtake : {
                    mesh_normal_unilateral_filtering_belyaev_ohtake(mesh, sigma_g, sigma_p, sigma_n,
                                                                    state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::unilateral_yagou_mean : {
                    mesh_normal_unilateral_filtering_yagou_mean(mesh, sigma_g, sigma_p, sigma_n,
                                                                state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::unilateral_yagou_median : {
                    mesh_normal_unilateral_filtering_yagou_median(mesh, sigma_g, sigma_p, sigma_n,
                                                                  state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::unilateral_yadav : {
                    mesh_normal_unilateral_filtering_yadav(mesh, sigma_g, sigma_p, sigma_n,
                                                           state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::unilateral_shen : {
                    mesh_normal_unilateral_filtering_shen(mesh, sigma_g, sigma_p, sigma_n,
                                                          state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::unilateral_tasdizen : {
                    mesh_normal_unilateral_filtering_tasdizen(mesh, sigma_g, sigma_p, sigma_n,
                                                              state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::unilateral_centin : {
                    mesh_normal_unilateral_filtering_centin(mesh, sigma_g, sigma_p, sigma_n,
                                                            state->config.parallel_grain_size);
                    break;
                }
                ...
                case NormalFilteringType::bilateral_zheng : {
                    mesh_normal_bilateral_filtering_zheng(mesh, sigma_g, sigma_p, sigma_n,
                                                          state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::bilateral_zhang : {
                    mesh_normal_bilateral_filtering_zhang(mesh, sigma_g, sigma_p, sigma_n,
                                                          state->config.parallel_grain_size);
                    break;
                }
                case NormalFilteringType::bilateral_yadav : {
                    mesh_normal_bilateral_filtering_yadav(mesh, sigma_g, sigma_p, sigma_n,
                                                          state->config.parallel_grain_size);
                    break;
                }
