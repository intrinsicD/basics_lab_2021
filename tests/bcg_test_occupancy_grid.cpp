...
TEST(TestOccupancyGrid, has_add_remove) {
    ...
    for (size_t i = 0; i < 100; ++i) {
        ...
        EXPECT_FALSE(grid.is_occupied(items.back()));
        grid.mark_occupied(items.back());
        EXPECT_TRUE(grid.is_occupied(items.back()));
        grid.mark_free(items.back());
    }

    for (const auto &item : items) {
        EXPECT_FALSE(grid.is_occupied(item));
    }
...
TEST(TestOccupancyGrid, traverse) {
    ...
    traverse(grid, [&has_any, &count](size_t idx, occupancy_grid &grid) {
        ++count;
        has_any = grid.is_occupied(idx);
        return true;
    });
    EXPECT_EQ(count, grid.capacity());
    EXPECT_FALSE(has_any);

    grid.mark_occupied(grid.aabb.min);
    grid.mark_occupied(grid.aabb.center());
    grid.mark_occupied(grid.aabb.max);
    ...
    grid.mark_free(grid.aabb.center());
...
TEST(TestOccupancyGrid, query_knn) {
    ...
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 1, 1)));
    grid.mark_occupied(VectorI<3>(1, 1, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 1, 2)));
    grid.mark_occupied(VectorI<3>(1, 1, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 1, 3)));
    grid.mark_occupied(VectorI<3>(1, 1, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 2, 1)));
    grid.mark_occupied(VectorI<3>(1, 2, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 2, 2)));
    grid.mark_occupied(VectorI<3>(1, 2, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 2, 3)));
    grid.mark_occupied(VectorI<3>(1, 2, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 3, 1)));
    grid.mark_occupied(VectorI<3>(1, 3, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 3, 2)));
    grid.mark_occupied(VectorI<3>(1, 3, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(1, 3, 3)));
    grid.mark_occupied(VectorI<3>(1, 3, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 1, 1)));
    grid.mark_occupied(VectorI<3>(2, 1, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 1, 2)));
    grid.mark_occupied(VectorI<3>(2, 1, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 1, 3)));
    grid.mark_occupied(VectorI<3>(2, 1, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 2, 1)));
    grid.mark_occupied(VectorI<3>(2, 2, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 2, 2)));
    grid.mark_occupied(VectorI<3>(2, 2, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 2, 3)));
    grid.mark_occupied(VectorI<3>(2, 2, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 3, 1)));
    grid.mark_occupied(VectorI<3>(2, 3, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 3, 2)));
    grid.mark_occupied(VectorI<3>(2, 3, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(2, 3, 3)));
    grid.mark_occupied(VectorI<3>(2, 3, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 1, 1)));
    grid.mark_occupied(VectorI<3>(3, 1, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 1, 2)));
    grid.mark_occupied(VectorI<3>(3, 1, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 1, 3)));
    grid.mark_occupied(VectorI<3>(3, 1, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 2, 1)));
    grid.mark_occupied(VectorI<3>(3, 2, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 2, 2)));
    grid.mark_occupied(VectorI<3>(3, 2, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 2, 3)));
    grid.mark_occupied(VectorI<3>(3, 2, 3));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 3, 1)));
    grid.mark_occupied(VectorI<3>(3, 3, 1));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 3, 2)));
    grid.mark_occupied(VectorI<3>(3, 3, 2));
    EXPECT_FALSE(grid.is_occupied(VectorI<3>(3, 3, 3)));
    grid.mark_occupied(VectorI<3>(3, 3, 3));


    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 1, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 1, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 1, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 2, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 2, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 2, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 3, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 3, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(1, 3, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 1, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 1, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 1, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 2, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 2, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 2, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 3, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 3, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(2, 3, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 1, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 1, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 1, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 2, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 2, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 2, 3)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 3, 1)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 3, 2)));
    EXPECT_TRUE(grid.is_occupied(VectorI<3>(3, 3, 3)));
